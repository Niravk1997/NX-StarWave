    ...
    public partial class Measurement_Plot_Window : MetroWindow
    {
        ...
        private void Show_Tracker_Click(object sender, RoutedEventArgs e)
        {
            if (Show_Tracker.IsChecked == true)
            {
                ...
                Mouse_Tracker.VerticalLine.PositionFormatter = x => DateTime.FromOADate(x).ToString("yyyy-MM-dd h:mm:ss.fff tt");
                Mouse_Tracker.HorizontalLine.PositionFormatter = y => Axis_Scale_Config.Value_SI_Prefix(y, 2) + Measurement_Unit;
        ...
        private void Graph_MouseMove(object sender, MouseEventArgs e)
        {
            if (ShowMouseTracker == true)
            {
                ...
                if (ShowMouseCoordinates == true)
                {
                    MouseCoordinates.Label = DateTime.FromOADate(X_MouseCoordinate).ToString("yyyy-MM-dd h:mm:ss.fff tt") + ", " + Axis_Scale_Config.Value_SI_Prefix(Y_MouseCoordinate, 4) + Measurement_Unit;
