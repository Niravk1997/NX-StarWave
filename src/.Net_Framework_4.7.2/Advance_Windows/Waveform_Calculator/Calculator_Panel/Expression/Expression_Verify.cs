...
namespace Waveform_Calculator
{
    public partial class Calculator_Panel : MetroWindow
    {
        ...
        private bool Verify_Math_Expression(Expression_Data expression_unverified)
        {
            try
            {
                if (new[] { "x1", "x2", "x3", "x4" }.Any(c => expression_unverified.Expression.Contains(c)))
                {
                    double[] Dummy_Data = { 1, 2, 3, 4, 5 };
                    double[] Results = new double[5];

                    Argument[] argument = new Argument[4];
                ...
                {
                    insert_Log("No Channel parameters (x1, x2, x3, x4) found in the math expression.", 2);
